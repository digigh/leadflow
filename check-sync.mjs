...
async function checkSync() {
    ...
    console.log(`Parsed ${metaLeads.length} leads from Meta`);

    if (metaLeads.length) {
        console.log('Sample parsed lead:', metaLeads[0]);
    }

    const { data: existing } = await supabase.from('leads').select('email, phone, lead_name');
    console.log(`Found ${existing?.length || 0} existing leads in Supabase`);
