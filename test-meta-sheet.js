...
const fetchSheet = async (sheetName) => {
    try {
        ...
        return { rowsCount: data.table.rows.length, sampleRows: data.table.rows.slice(0, 5) };
    } catch (e) {
...
async function test() {
    ...
    fs.writeFileSync('meta-sheet-test-results.json', JSON.stringify(results, null, 2));
    console.log("Done");
}
