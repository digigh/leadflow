...
async function checkSync() {
    ...
    const newLeads = metaLeads.filter(l => {
        // Strip out non-digits from both sides for phone compare
        const cleanPhone = l.phone?.replace(/\D/g, '') || '';

        return !existing?.some(e => {
            const eCleanPhone = e.phone?.replace(/\D/g, '') || '';

            return (l.email && e.email !== '' && e.email === l.email) ||
                (cleanPhone && eCleanPhone !== '' && eCleanPhone === cleanPhone && e.lead_name === l.lead_name);
        });
    });

    console.log(`After deep deduplication, ${newLeads.length} NEW leads will be inserted.`);
}
